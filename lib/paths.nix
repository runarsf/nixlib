{lib, ...}: let
  inherit (builtins) filter map match toString readDir elem dirOf baseNameOf;
    ...
    with fileset; let
      ...
      isExcluded = path:
        ...
        then true
          ...
          (filter (excludedDir: hasPrefix excludedDir path)
            excludedDirs)
          != [];

      myFiles = unique ((filter (file:
          pathIsRegularFile file
          && hasSuffix ".nix" (toString file)
      ...
      dirsWithDefaultNix =
        filter (dir: elem dir dirOfFile)
        (map (file: dirOf file) (filter (file:
          match "default.nix" (baseNameOf file) != null)
        myFiles));

      filteredFiles = filter (file:
        elem (dirOf file) dirsWithDefaultNix
        == false
        || match "default.nix" (baseNameOf file) != null)
      myFiles;
    in
